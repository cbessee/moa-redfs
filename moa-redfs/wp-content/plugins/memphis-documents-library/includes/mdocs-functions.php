...
function mdocs_get_num_cols() {
	...
	// =============== CRITIGEN CUSTOMIZATION ===================
	// Add 2 to column total as we are adding 2 extra columns
	return $num_cols + 2;
}
/**
...
function mdocs_get_parents($child, $permalink='', $att=null, $tabs = array('parent-tab' => '', 'current-tab' => '&emsp;&emsp;', 'child-tab' => '&emsp;&emsp;&emsp;&emsp;')) {
	...
	$html = '
			<div class="mdocs-parent-cat" >
				<div id="title" class="mdocs-tooltip">
...
/* ================= CRITIGEN CUSTOMIZATIONS =================== */
// Restructure current sub/parent folder displays to be more clear
	...
	echo $current_cat_string.$child_cats_string."</div><div class='folder-title'><i class='fa fa fa-folder-open-o' aria-hidden='true'></i> ".$current_cat['name']."</div>";
...
function mdocs_list_header($show=true) {
	...
	$parent = mdocs_get_the_folder($current_cat);
	while ($parent && $parent['depth'] >= 1) {
		$parent = mdocs_get_the_folder($parent['parent']);
	}
	if($show) {
		global $post;
		$mdocs = get_option('mdocs-list');
				...
				<nav class="navbar mdocs-navbar-default" role="navigation" id="mdocs-navbar">
					<div class="container-fluid">
						<div class="navbar-header">
							...
							<div class="navbar-brand" style="text-align: center; width: 100%;"><?php _e('Top-Level Folders','memphis-documents-library'); ?></div>
						</div>
							...
							<ul class="nav navbar-nav">
								...
								if(!empty($cats)) {
									foreach( $cats as $index => $cat ){
										...
										else echo '<li class="'.($current_cat == $cat['slug'] || $parent['slug'] == $cat['slug'] ? 'active' : '').'"><a href="'.$permalink.$cat['slug'].'">'.__($cat['name']).'</a></li>';
									}
								}
								?>
							</ul>
