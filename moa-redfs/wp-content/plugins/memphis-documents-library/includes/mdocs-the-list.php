...
/* Unsortable table headings for both the thumbnails and document metadata has been added to the header and footer */
...
?><div class="mdocs-container"><?php
	...
	/* =========== CRITIGEN CUSTOMIZATIONS =============== */
	// Move sub cats up, outside of the table
	// SUB CATEGORIES
	...
	if($display_files) {
		...
		<tr class="hidden-sm hidden-xs">
			...
			<th class="mdocs-sort-option" data-disable-user-sort="1" ><?php _e('Metadata Tags','memphis-documents-library'); ?></th>
			<?php
		...
		<?php
			...
			<tr class="hidden-sm hidden-xs">
				...
				<th class="mdocs-sort-option" data-disable-user-sort="<?php echo $disable_user_sort; ?>" data-sort-type="name" data-current-cat="<?php echo $current_cat; ?>" data-permalink="<?php echo $permalink; ?>"><?php _e('Name','memphis-documents-library'); ?><?php if($mdocs_sort_type == 'name') echo $mdocs_sort_style_icon; ?></th>
				<th class="mdocs-sort-option" data-disable-user-sort="1" ><?php _e('Thumbnail','memphis-documents-library'); ?></th>
				<th class="mdocs-sort-option" data-disable-user-sort="1" ><?php _e('Metadata Tags','memphis-documents-library'); ?></th>
				<?php
				foreach(get_option('mdocs-displayed-file-info') as $key => $option) {
					if(isset($option['show']) && $option['show']) {
		...
		if($has_one_file == false && get_option('mdocs-show-no-file-found')) {
			?><tr><td colspan="<?php echo $num_tds + 1; ?>"><p class="mdocs-nofiles" ><?php _e('No files found in this folder.','memphis-documents-library'); ?></p></td></tr><?php
		}
		
		?></table></div></div><?php
	} else {
		if($has_one_file == false && get_option('mdocs-show-no-file-found')) {
			?><p class="mdocs-nofiles" ><?php _e('No files found in this folder.','memphis-documents-library'); ?></p><?php
		}
	}
