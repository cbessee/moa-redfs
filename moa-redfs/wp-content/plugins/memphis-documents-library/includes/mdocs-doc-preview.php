...
function mdocs_show_description($id) {
	...
	if(mdocs_check_file_rights($the_mdoc)) {
		...
		echo "<div style='padding: 10px 0;'></div>";
		...
		/* Add categories and tags to document preview and post as metadata */

		if($categories || $tags) { 
			echo "<h4>Document Metadata</h4>";
			foreach($categories as $cat) {
				echo "<p style='padding-left: 10px; margin-bottom: 0px;' class='cat'>".$cat->name."</p>";
			}
			foreach($tags as $tag) {
				echo "<p style='padding-left: 10px; margin-bottom: 0px;' class='tag'>".$tag->name."</p>";
			}
		}
